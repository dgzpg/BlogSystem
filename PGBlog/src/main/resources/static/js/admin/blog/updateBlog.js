	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data != null) {
				...
				var imgPath = '<img class="picPath animated fadeInRight"  style="width:100%" alt="封面" title="点击更换封面" src="' + data.blog.picturePath + '" />';
				$(".imagePath").val(data.pictureId);
				$(".picPath").html(imgPath);
				//加载编辑页面预览的  时间和浏览量
				$(".newsview").find(".au02").html(data.blog.publishedTime);
				$(".au03").find('b').html(data.blog.pageView);

				$("#clicknum").html(data.blog.pageView);
				...
				var recommend = '否';
				if (data.blog.recommend == 1) {
					recommend = '是';
				}
				$("#istop").html(top);
				$("#isrecommend").html(recommend);
				
				var typeName = '';
				typeName += '<option style="color:#000;font-weight:bold;" value="' + data.blogTypeId + '">' + data.blog.type + '</option>';
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				typeName += '<option value="' + data[i].typeId + '">' + data[i].type + '</option>';
			}
			// 初始化数据
			$(".form-horizontal").find('select[name=typeName]').append(typeName);
		},
	...
	$.ajax({
		...
		success : function(data) {
			if (data.size == 0) {
				...
				var pics = '';
				for (var i = 0; i < data.length; i++) {
					pics += '<a class="fancybox" href="javascript:void(0);"><img onclick="selectImgPath(this)" style="width: 190px; height: 115px;float:left;margin-right:3px;" id="'+data[i].id+'"alt="image" src="' + data[i].path + '" /></a>'
				}
			}
			$(".picsList").html(pics);
		},
	...
	$.ajax({
		...
		success : function(data) {
			if (data ==1) {
				...
				setTimeout(function() {
					window.location.href = "toBlogTable";
				}, 1500);
			} else {
