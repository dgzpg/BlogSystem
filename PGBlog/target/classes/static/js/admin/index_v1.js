...
var initResourceCountByStatus = function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$(".resource").html(data[0]);
		
			var draftPercent = Math.round(data[0] / allCount * 100) / 1.00 + '%<i class="fa fa-bolt"></i>';
			$(".resourcePercent").html(draftPercent);
		
			globalCount++;
			returnAllCount();
		},
		error : function() {
			swal("资源总数错误", "请重新操作", "error");
		}
	});
};
...
var initBlogCountByStatus = function() {
	...
	$.ajax({
		...
		dataType : 'json',
		success : function(data) {
			var allCount = data[0]+data[1]+data[2];
...
var initVisitCount = function(e) {
	...
	$.ajax({
		url : 'selectVisitForToday',
		type : 'get',
		dataType : 'json',
		success : function(data) {
			$(".nowVisitors").html(data[0]);
			var nowVisitorsPercent = level(data[0], data[1]);
			$(".nowVisitorsPercent").html(nowVisitorsPercent);
			
			$(".visitors").html(data[2]);
		...
		success : function(data) {
			...
			for (var j = 0; j < days.length; j++) {
				for (var i = 0; i < data.num; i++) {
					var time = Format(data.days[i], "yyyy-MM-dd");
					if (days[j] == time) {
						counts[j] = data.count[i];
					}
				}
			}
			if (num > 10) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var j = 0; j < arr.length; j++) {
				for (var i = 0; i < data.num; i++) {
					var time = Format(data.days[i], "yyyy-MM");
					if (arr[j] == time) {
						counts[j] = data.count[i];
					}
				}
...
var initVisitCountByYear = function(num) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var j = 0; j < arr.length; j++) {
				for (var i = 0; i < data.num; i++) {
					var time = Format(data.days[i], "yyyy");
					if (arr[j] == time) {
						counts[j] = data.count[i];
					}
				}
