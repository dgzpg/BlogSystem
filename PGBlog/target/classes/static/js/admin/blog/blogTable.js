...
//草稿/发布...按钮绑定查询事件  
//params中的status的值：（ 1 发布） ，（2 垃圾箱），（3 推荐）  （4 置顶） （5 所有的） （6 类别） 
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				typeName += '<li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;" onclick="sendType(' + data[i].typeId + ')">' + data[i].type + '</a></li>';
			}
			// 初始化数据
...
var selectBlog = function() {
	...
	$('#allBlog').bootstrapTable({
		method : 'get',
		url : "selectGroupLikeBlogTableByPage",
		dataType : "json",
		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.blogId + '">' + index1 + '</span>';
					return id;
				}
			},
			{
			...
			},
			{
				...
				formatter : function(value, row, index) {
					//查看	
					var a = '<a  class=" btn-sm btn-info" href="#" data-toggle="modal" data-target="#myModal" onclick="selectBlogById(\'' + row.blogid + '\')">查看</a> ';
					//编辑
					var b = '<a  class=" btn-sm btn-primary"  href="/edit?id=' + row.blogid + '"><i class="fa fa-edit" ></i> 编辑</a> ';
					//从回收站还原
					var c = '<a  class=" btn-sm btn-danger"  onclick="operation(' + row.blogid + ',\'还原\')"><i class="fa fa-share-square-o" ></i> 还原</a> ';
					//取消推荐
					var d = '<a  class=" btn-sm btn-primary"  onclick="operation(' + row.blogid + ',\'取推荐\')">取 推荐</a> ';
					//从草稿还原
					var e = '<a  class=" btn-sm btn-danger"  onclick="operation(' + row.blogid + ',\'发表\')"><i class="fa fa-share-square-o" ></i> 发表</a> ';
					//取消置顶
					var f = '<a  class=" btn-sm btn-success"  onclick="operation(' + row.blogid + ',\'取置顶\')">取 置顶</a> ';
					
					var g = '<a  class=" btn-sm btn-success"  onclick="operation(' + row.blogid + ',\'删除\')">删除</a> ';
					if (row.status == 2) {
						return  c + g;
					}
					else if (row.status == 1) 
	...
	swal({
		...
		if (status == 1) {
			for (var i = 0; i < date.length; i++) {
				idArray[i] = date[i].blogid;
				operationBlog(idArray[i], 2, null, null); //参数2表示  放入回收站
			}
		} else if (status == 2) {
			for (var i = 0; i < date.length; i++) {
				idArray[i] = date[i].blogid;
				operationBlog(idArray[i], null, null, 2); // 设置为置顶
			}
		} else if (status == 3) {
			for (var i = 0; i < date.length; i++) {
				idArray[i] = date[i].blogid;
				operationBlog(idArray[i], null, 2, null); //设置为推荐
			}
		}
	});
};
...
var operation = function(id, op) {
	...
	swal({
		...
		if (op == "还原") {
			...
			operationBlog(id, null, 1, null)
		} else if (op == "发表") {
			operationBlog(id, 1, null, null)
		} else if (op == "取置顶") {
			operationBlog(id, null, null, 1)
		} else if(op=="删除"){
			deleteBlog(id)
		}
	});
...
{
	var param='将ID为<span class="text-info">' + id + '</span>的博客<span class="text-danger">删除</span>';
	param={
			param : param,
			user : 1,
			id : id,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data!= null) {
				...
				var update = '<a  class="J_menuItem btn btn-white" href="edit?id='+data.blog.blogid+'">编辑</a>';
