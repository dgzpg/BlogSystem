...
var initType = function() {
	...
	$.ajax({
		...
		success : function(data) {
			var typeNameAndNum = '';
			var circle = new Array("text-navy", "text-da;nger", " text-info", "text-primary", "text-warning")
			var label = new Array("label-primary", "label-danger", " label-info", "label-success", "label-warning");
			for (var i = 0; i < data.length; i++) {
				typeNameAndNum += '<li><a href="javascript:void(0);"> <i class="fa fa-circle ' + circle[i % 5] + '"></i> ' +data[i].type + '<span class="label ' + label[i % 5] + ' pull-right">' + data[i].typeNum + ' 篇</span></a></li>'
			}
			// 初始化数据
		...
		columns : [
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.typeId + '">' + index1 + '</span>';
					return id;
				}
			},
			{
			...
			},
			{
				...
				width : '12%',
				formatter : function(value, row, index) {
					var a = '<a  class=" btn-sm btn-info" data-toggle="modal" data-target="#modal-form" onclick="selectBlogTypeById(' + row.typeId + ')"><i class="fa fa-edit" ></i> 编辑</a> ';
					var b = '<a  class=" btn-sm btn-danger"   onclick="deleteBlogType(' + row.typeId + ',\'' + row.type + '\')"><i class="fa fa-trash-o" ></i> 删除</a> ';
					return a + b;
				}
			}
		]
...
var selectBlogTypeById = function(id) {
	...
	$.ajax({
		...
		success : function(data) {
			if (data != null) {
				$("#oldTypeName").html(data.type);
				var updateButton = ' <button class="btn btn-sm btn-primary pull-right m-t-n-xs" onclick="updateBlogType(' + data.typeId + ',\'' + data.type + '\')" type="button"><strong>提交</strong></button>'
