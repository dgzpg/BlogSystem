		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					//删除
					var b = '<a  class=" btn-sm btn-danger"   onclick="deleteResource(' + row.id + ',\'' + row.name + '\')"><i class="fa fa-trash-o" ></i> 删除</a> ';
					//下架
					var c = '<a  class=" btn-sm btn-danger"  title="下架后前台将无法展示" onclick="updateResource(' + row.id + ',\'' + row.name + '\',2)"><i class="fa fa-hand-o-down" ></i> 下架</a> ';
					//上架
					var d = '<a  class=" btn-sm btn-primary"  title="上架后将在前台展示" onclick="updateResource(' + row.id + ',\'' + row.name + '\',1)"><i class="fa fa-hand-o-up" ></i> 上架</a> ';
					if (row.status == 2) {
						return d + b;
					} else if (row.status == 1) {
						return a + c;
	...
	$.ajax({
		...
		success : function(data) {
			if (data != null) {
				...
				$("#newLink").val(data.src);
				$("#newPassword").val(data.password);
				var updateButton = ' <button class="btn btn-sm btn-primary pull-right m-t-n-xs" onclick="updateResource(' + data.id + ',\'' + data.name + '\',3)" type="button"><strong>提交</strong></button>'
