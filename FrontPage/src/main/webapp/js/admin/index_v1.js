...
var initResourceCountByStatus = function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$(".resource").html(data.publishedNum);
		
			var draftPercent = Math.round(data.publishedNum / allCount * 100) / 1.00 + '%<i class="fa fa-bolt"></i>';
...
var initVisitCount = function(e) {
	...
	$.ajax({
		...
		dataType : 'json',
		success : function(data) {
			$(".nowVisitors").html(data.todayVisitores);
			var nowVisitorsPercent = level(data.todayVisitores, data.yesterdayVisitores);
			$(".nowVisitorsPercent").html(nowVisitorsPercent);
			
			$(".visitors").html(data.allVisitores);
