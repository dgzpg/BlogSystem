...
var selectPrevAndNextBlog = function() {
  var id = $(".id").val();
  var params = {
    id : id
  };
  $.ajax({
    url : '../selectPrevAndNextBlog',
    type : 'get',
    data : params,
    dataType : 'json',
    success : function(data) {
      var preTitle = "";
      if (data[0]!=null) {
        var id = data[0].blog.blogid;
        preTitle = '<a href="selectBlogById?id=' + id + '&user=2">' + data[0].blog.title + '</a>';
      } else {
        preTitle = '<span>无</span>';
      }
      if (data[1] != null) {
          var sid = data[1].blog.blogid;
          nextTitle = '<a href="selectBlogById?id=' + sid + '&user=2">' + data[1].blog.title + '</a>';
        } else {
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        relBlog += '<li><i style="color:#5788aa" class="fa fa-paperclip"></i> <a href="selectBlogById?id=' + id + '&user=2" title="' + data[i].title + '">' + data[i].title + '</a></li>'
      }
      // 初始化数据
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var id = data.id;
        time = i * 0.05;
        likeBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeIn"><b><a href="selectBlogById?id=/' + id + '&user=2">'
          + data[i].title
  ...
  $.ajax({
    ...
    success : function(data) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var id = data[i].blogid;
        time = i * 0.05;
        clickBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeIn"><b><a href="selectBlogById?id=' + id + '&user=2">'
          + data[i].title
          + '</a></b><p><i><img src="' + data[i].picturePath + '"></i><span>'
          + data[i].introduction + '</span></p></li>'
      }
